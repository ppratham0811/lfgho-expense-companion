...
contract GhoSteward is Ownable, IGhoSteward {
    ...
    function updateBorrowRate(uint256 newBorrowRate) external onlyRiskCouncil {
        ...
        uint256 oldBorrowRate = GhoInterestRateStrategy(
            ghoReserveData.interestRateStrategyAddress
        ).getBaseVariableBorrowRate();
        require(
            _borrowRateChangeAllowed(oldBorrowRate, newBorrowRate),
        ...
        IPoolConfigurator(
            IPoolAddressesProvider(POOL_ADDRESSES_PROVIDER)
                ...
                GHO_TOKEN,
                cachedStrategyAddress
            );
    }

    /// @inheritdoc IGhoSteward
    function updateBucketCapacity(
        uint128 newBucketCapacity
    ) external onlyRiskCouncil {
        require(block.timestamp < _stewardExpiration, "STEWARD_EXPIRED");
        require(
            block.timestamp - _timelocks.bucketCapacityLastUpdated >
                MINIMUM_DELAY,
            "DEBOUNCE_NOT_RESPECTED"
        );
        ...
        require(
            ghoReserveData.aTokenAddress != address(0),
            "GHO_ATOKEN_NOT_FOUND"
        );

        (uint256 oldBucketCapacity, ) = IGhoToken(GHO_TOKEN)
            .getFacilitatorBucket(ghoReserveData.aTokenAddress);
        require(
            _bucketCapacityIncreaseAllowed(
                oldBucketCapacity,
                newBucketCapacity
            ),
     ...
     */
    function _borrowRateChangeAllowed(
        uint256 from,
        uint256 to
     ...
     */
    function _bucketCapacityIncreaseAllowed(
        uint256 from,
        uint256 to
    ) internal pure returns (bool) {
