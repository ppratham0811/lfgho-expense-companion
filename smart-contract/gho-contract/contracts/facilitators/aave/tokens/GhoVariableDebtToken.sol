 ...
 */
contract GhoVariableDebtToken is
    DebtTokenBase,
    ...
    IGhoVariableDebtToken
{
    ...
    modifier onlyDiscountToken() {
        require(
            address(_discountToken) == msg.sender,
    ...
    function balanceOf(
        ...
        if (discountPercent != 0) {
            uint256 balanceIncrease = balance -
                scaledBalance.rayMul(previousIndex);
    ...
    function updateDiscountRateStrategy(
        ...
        require(
            newDiscountRateStrategy != address(0),
            "ZERO_ADDRESS_NOT_VALID"
        );
        address oldDiscountRateStrategy = address(_discountRateStrategy);
        _discountRateStrategy = IGhoDiscountRateStrategy(
    ...
    /// @inheritdoc IGhoVariableDebtToken
    function getDiscountRateStrategy()
        external
        view
        override
        returns (address)
    ...
    function updateDiscountDistribution(
        ...
        if (
            senderPreviousScaledBalance == 0 &&
            recipientPreviousScaledBalance == 0
        ) {
            return;
        }
    ...
    function decreaseBalanceFromInterest(
        ...
        _ghoUserState[user].accumulatedDebtInterest = (_ghoUserState[user]
            .accumulatedDebtInterest - amount).toUint128();
    }
    ...
    function _refreshDiscountPercent(
        ...
        uint256 newDiscountPercent = _discountRateStrategy
            .calculateDiscountRate(balance, discountTokenBalance);
