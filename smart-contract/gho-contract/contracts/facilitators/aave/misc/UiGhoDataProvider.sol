...
contract UiGhoDataProvider is IUiGhoDataProvider {
    ...
    function getGhoReserveData()
        public
        view
        override
        returns (GhoReserveData memory)
    {
        DataTypes.ReserveData memory baseData = POOL.getReserveData(
            address(GHO)
        );
        IGhoVariableDebtToken debtToken = IGhoVariableDebtToken(
        ...
        (uint256 bucketCapacity, uint256 bucketLevel) = GHO
            .getFacilitatorBucket(baseData.aTokenAddress);

        return
            GhoReserveData({
                ghoBaseVariableBorrowRate: baseData.currentVariableBorrowRate,
                ghoDiscountedPerToken: discountRateStrategy
                    .GHO_DISCOUNTED_PER_DISCOUNT_TOKEN(),
                ghoDiscountRate: discountRateStrategy.DISCOUNT_RATE(),
                ghoMinDebtTokenBalanceForDiscount: discountRateStrategy
                    .MIN_DEBT_TOKEN_BALANCE(),
                ghoMinDiscountTokenBalanceForDiscount: discountRateStrategy
    ...
    /// @inheritdoc IUiGhoDataProvider
    function getGhoUserData(
        address user
    ) public view override returns (GhoUserData memory) {
        DataTypes.ReserveData memory baseData = POOL.getReserveData(
            address(GHO)
        );
        IGhoVariableDebtToken debtToken = IGhoVariableDebtToken(
